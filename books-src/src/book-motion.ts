...
export type BrowseMotionPhase = "retreat-current" | "turn-current" | "shelve-current" | "extract-next" | "turn-next" | "settle-next";
...
export function browseMotionPose(phase: BrowseMotionPhase, progress: number, layout: MotionLayout): BookPose {
...
export function focusedBookPose(progress: number, layout: MotionLayout, focusX: number, focusZ: number, focusScale: number): BookPose {
...
export function bookFootprintsOverlap(left: BookFootprint, right: BookFootprint, margin = collisionMargin) {
  const leftAxes = axesFor(left);
  const rightAxes = axesFor(right);
  const axes = [leftAxes.width, leftAxes.thickness, rightAxes.width, rightAxes.thickness];
  const centerDelta = {
    x: right.x - left.x,
    z: right.z - left.z,
  };
