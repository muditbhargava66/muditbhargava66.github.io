...
import { createBackCover, createFrontCover, createSpineCover, createTitleDecal } from "./cover-art";
import { STRIPE_ASSET_ROOT, stripeAssetUrl, type StripeBookAsset } from "./stripe-assets";
import { addStripeFoilBlend, stripeFoilSettings } from "./stripe-foil";
...
const shelfTopGeneral = -0.65;
const shelfClearance = 0.5;
const browseCamera = new THREE.Vector3(0, 0.72, 9.2);
const browseTarget = new THREE.Vector3(0, 0.48, 0.15);
...
function toTexture(canvas: HTMLCanvasElement, renderer: THREE.WebGLRenderer, anisotropy = 8) {
...
export class ShelfEngine {
  ...
  private stripeTextureCache = new Map<string, Promise<THREE.Texture | null>>();
  ...
  private browseLookTarget = browseTarget.clone();
  private shelfTopTech = 0.8;
  private maxTechHeight = 0;
  private maxGeneralHeight = 0;
  private vPan = 0.5;
  private targetVPan = 0.5;
  private vPanInitialized = false;
  private viewTop = 3.2;
  private viewBottom = -1.25;
  private yTechCenter = 1.7;
  private yGenCenter = 0.3;
  private dOverview = 12;
  private dTech = 6;
  private dGen = 6;
  private wallLabels: THREE.MeshBasicMaterial[] = [];
  private lastTimestamp = 0;
  private lastDiagnosticsAt = 0;
  private isDisposed = false;

  constructor(canvas: HTMLCanvasElement, books: CatalogBook[], callbacks: ShelfCallbacks) {
    this.canvas = canvas;
  ...
  private setupScene() {
    ...
    this.scene.fog = new THREE.Fog("#eee8db", 14, 32);
  ...
  private createBooks() {
    ...
    this.booksData.forEach((book) => {
      if (book.category === "tech") {
        this.maxTechHeight = Math.max(this.maxTechHeight, book.height);
      } else {
        this.maxGeneralHeight = Math.max(this.maxGeneralHeight, book.height);
      }
    });
    this.shelfTopTech = shelfTopGeneral + this.maxGeneralHeight + shelfClearance;
    this.yTechCenter = this.shelfTopTech + this.maxTechHeight * 0.5;
    this.yGenCenter = shelfTopGeneral + this.maxGeneralHeight * 0.5;
    this.viewTop = this.shelfTopTech + this.maxTechHeight + 0.3;
    this.viewBottom = shelfTopGeneral - 0.6;

    this.booksData.forEach((book, index) => {
      const isTech = book.category === "tech";
      let x = 0;
      let y = isTech ? this.shelfTopTech : shelfTopGeneral;

      if (isTech) {
        cursorTech += book.thickness * 0.5;
        x = cursorTech;
      } else {
        cursorGeneral += book.thickness * 0.5;
        x = cursorGeneral;
      }

      const runtime = this.createBook(book, index, x, y);
      ...
      if (isTech) {
        cursorTech += book.thickness * 0.5 + gap;
      } else {
        cursorGeneral += book.thickness * 0.5 + gap;
      }
    });
    ...
    this.runtimeBooks.forEach((book, index) => {
      this.commitBookPose(book, index === 0 ? presentedBookPose(this.motionLayout) : shelvedBookPose(this.motionLayout), false);
    });

    const buildShelf = (cursor: number, yLevel: number) => {
      ...
      shelf.position.set(cursor * 0.5, yLevel - 0.14, 0);
      ...
      shelfEdge.position.set(cursor * 0.5, yLevel - 0.08, 0.85);
      shelfEdge.castShadow = true;
      this.shelfFurniture.add(shelfEdge);
    };

    buildShelf(cursorTech, this.shelfTopTech);
    buildShelf(cursorGeneral, shelfTopGeneral);

    // Shelf category labels on the wall
    const createLabelTexture = (text: string): THREE.CanvasTexture => {
      ...
      ctx.font = '600 26px "Iowan Old Style", "Palatino Linotype", "Book Antiqua", Palatino, serif';
      ctx.fillStyle = "#8a7b6b";
      ctx.letterSpacing = "3px";
      ctx.textAlign = "left";
      ctx.textBaseline = "middle";
      ctx.fillText(text.toUpperCase(), 16, 34);
      const tex = new THREE.CanvasTexture(canvas);
      tex.colorSpace = THREE.SRGBColorSpace;
      return tex;
    };

    const createLabel = (text: string, yCenter: number) => {
      ...
      const labelMesh = new THREE.Mesh(new THREE.PlaneGeometry(3.2, 0.4), material);
      labelMesh.position.set(-1.5, yCenter, -3.0);
      this.shelfFurniture.add(labelMesh);
      this.wallLabels.push(material);
    };

    createLabel("Coding & Tech", this.shelfTopTech + 2.45);
    createLabel("General Reading", shelfTopGeneral + this.maxGeneralHeight + 0.05);
  }

  private createBook(book: CatalogBook, index: number, x: number, y: number): RuntimeBook {
    const width = 1.31 + ((index % 5) - 2) * 0.018;
    const depth = book.thickness;
    const slot = new THREE.Group();
    slot.name = `bookSlot:${book.id}`;
    slot.position.set(x, y + book.height * 0.5, 0.04);
    ...
    const pageBlock = new THREE.Mesh(
      new RoundedBoxGeometry(width - 0.075, book.height - 0.105, Math.max(0.08, depth - 0.052), 3, 0.018),
      paperMaterial
    );
    ...
    const spine = new THREE.Mesh(new RoundedBoxGeometry(0.055, book.height - 0.01, depth + 0.012, 3, 0.018), boardMaterial);
    spine.name = "spine";
    ...
    const frontSurface = new THREE.Mesh<THREE.PlaneGeometry, THREE.MeshPhysicalMaterial>(
    ...
    if (book.living) {
      ...
      const shimmer = new THREE.Mesh(new THREE.PlaneGeometry(width - 0.07, book.height - 0.07), livingMaterial);
  ...
  private handleWheel = (event: WheelEvent) => {
    ...
    if (Math.abs(event.deltaX) > Math.abs(event.deltaY)) {
      this.targetScrollIndex = clamp(this.targetScrollIndex + event.deltaX * 0.0024, 0, this.runtimeBooks.length - 1);
    } else {
      this.targetVPan = clamp(this.targetVPan + event.deltaY * 0.00075, 0, 1);
    }
  ...
  private handlePointerMove = (event: PointerEvent) => {
    ...
    if (this.pointerDown && event.pointerId === this.pointerId) {
      ...
      this.pointerLastY = event.clientY;
      this.pointerTravel += Math.abs(delta) + Math.abs(deltaY);
      ...
      this.targetVPan = clamp(this.targetVPan - deltaY * (1.6 / Math.max(1, this.canvas.clientHeight)), 0, 1);
      this.lastInputTime = performance.now();
  ...
  private xAtIndex(index: number) {
    ...
    return THREE.MathUtils.lerp(this.runtimeBooks[lower]?.x ?? 0, this.runtimeBooks[upper]?.x ?? 0, fraction);
  }

  private footprintFor(book: RuntimeBook, pose: BookPose = book.pose): BookFootprint {
    return {
      id: book.data.id,
      x: book.x + pose.x,
  ...
  private collisionFor(book: RuntimeBook, pose: BookPose) {
    ...
    return (
      this.runtimeBooks.find(
        (other) =>
          other !== book &&
          other.shelfY === book.shelfY &&
          bookFootprintsOverlap(proposed, this.footprintFor(other), this.motionLayout.collisionMargin)
      ) ?? null
    );
  }

  private commitBookPose(book: RuntimeBook, pose: BookPose, guardCollision = true) {
    if (guardCollision) {
  ...
  private updateBrowseMotion(delta: number) {
    ...
    const duration = this.reducedMotion ? Math.max(0.055, browsePhaseDuration[phase] * 0.45) : browsePhaseDuration[phase];
    const nextProgress = clamp(this.browseMotionProgress + delta / duration, 0, 1);
    const movingBook = this.runtimeBooks[motionIndex];
    const proposedPose = browseMotionPose(phase, nextProgress, this.motionLayout);
  ...
  private updateState(delta: number, timestamp: number) {
    if (this.mode === "browse") {
      ...
      this.vPan = damp(this.vPan, this.targetVPan, this.reducedMotion ? 20 : 10, delta);
      this.updateBrowseCamera();
      ...
      if (this.focusProgress <= 0) {
        if (this.selectedIndex !== null) {
          this.commitBookPose(this.runtimeBooks[this.selectedIndex], presentedBookPose(this.motionLayout));
  ...
  private updateBooks(delta: number, elapsed: number) {
    ...
    if (this.selectedIndex !== null) {
      ...
      this.commitBookPose(selected, focusedBookPose(motionFocus, this.motionLayout, focusX, focusZ, focusScale));
    }
  ...
  private applyFocusViewOffset(progress: number) {
    ...
    const verticalHalfSpan = Math.tan(THREE.MathUtils.degToRad(this.camera.fov * 0.5)) * focusDistance;
  ...
  private updateBrowseCamera() {
    const p = clamp(this.vPan, 0, 1);
    const y = THREE.MathUtils.lerp(this.yTechCenter, this.yGenCenter, p);
    const closeDistance = THREE.MathUtils.lerp(this.dTech, this.dGen, p);
    const towardEnd = clamp(Math.abs(2 * p - 1), 0, 1);
    const blend = towardEnd * towardEnd * (3 - 2 * towardEnd);
    const distance = THREE.MathUtils.lerp(this.dOverview, closeDistance, blend) + 0.5;
    this.responsiveBrowseCamera.set(0, y + 0.12, distance);
    this.browseLookTarget.set(0, y, 0.15);

    const viewHalf = distance * Math.tan(THREE.MathUtils.degToRad(this.camera.fov * 0.5)) * this.camera.aspect;
    const labelOpacity = THREE.MathUtils.smoothstep(viewHalf, 2.2, 3.2);
    this.wallLabels.forEach((material) => {
  ...
  private handleResize = () => {
    ...
    this.camera.fov = width < 600 ? 38 : width < 920 ? 34 : 30;
    this.camera.updateProjectionMatrix();

    const tanHalf = Math.tan(THREE.MathUtils.degToRad(this.camera.fov * 0.5));
    const halfSpan = (this.viewTop - this.viewBottom) * 0.5 + 0.2;
    this.dOverview = halfSpan / tanHalf;
    this.dTech = (this.maxTechHeight * 0.5 + 0.7) / tanHalf;
    this.dGen = (this.maxGeneralHeight * 0.5 + 0.7) / tanHalf;
    if (!this.vPanInitialized) {
      this.vPanInitialized = true;
      this.targetVPan = width / height < 0.9 ? 0 : 0.5;
      this.vPan = this.targetVPan;
    }
    this.updateBrowseCamera();
  ...
  private async loadStripeAssets() {
    try {
      ...
      const sourceMesh = parsed.children.find((child): child is THREE.Mesh => child instanceof THREE.Mesh);
  ...
  private textureFor(reference: { local_file: string | null } | undefined, color = false) {
    ...
    try {
      ...
      if (foil && foilSettings.enabled) {
        material.onBeforeCompile = (shader) => {
          ...
          shader.fragmentShader = addStripeFoilBlend(shader.fragmentShader);
        };
  ...
  private findAnyCollision(): [string, string] | null {
    for (let leftIndex = 0; leftIndex < this.runtimeBooks.length; leftIndex += 1) {
      ...
      for (let rightIndex = leftIndex + 1; rightIndex < this.runtimeBooks.length; rightIndex += 1) {
        ...
        if (
          left.shelfY === right.shelfY &&
          bookFootprintsOverlap(this.footprintFor(left), this.footprintFor(right), this.motionLayout.collisionMargin)
        ) {
