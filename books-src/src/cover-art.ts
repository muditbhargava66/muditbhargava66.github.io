...
function strokeLine(ctx: CanvasRenderingContext2D, points: Array<[number, number]>, width = 4) {
...
function drawMotif(ctx: CanvasRenderingContext2D, book: CatalogBook, width: number, height: number) {
  ...
  switch (book.motif) {
    case "lattice": {
      ...
      for (let i = 0; i < 24; i += 1) {
        nodes.push([110 + random() * (width - 220), 380 + random() * (height - 560)]);
      }
    ...
    case "branches": {
      ...
      for (let level = 0; level < 5; level += 1) {
        ...
        for (let index = 0; index < count; index += 1) {
          ...
          const parentX =
            level === 0 ? root[0] : width / 2 + (parentIndex / Math.max(1, parentCount - 1) - 0.5) * width * (0.13 + (level - 1) * 0.1) * 2;
    ...
    case "fracture": {
      ...
      for (let i = 0; i < 16; i += 1) {
        ...
        strokeLine(
          ...
          [
            [startX, startY],
            [(startX + endX) / 2 + (random() - 0.5) * 90, (startY + endY) / 2],
            [endX, endY],
          ],
          i % 4 === 0 ? 12 : 4
        );
...
function addPaperGrain(ctx: CanvasRenderingContext2D, width: number, height: number, seed: string) {
...
function drawCoverTypography(ctx: CanvasRenderingContext2D, book: CatalogBook, width: number, height: number, decalOnly = false) {
  ctx.save();
